...
# Test APOD Fetcher Lambda Function for Infinite v1.0
# This script tests the APOD fetcher function

set -e

echo "🧪 Testing APOD Fetcher Lambda function for Infinite v1.0..."
...
echo "🗄️ Checking DynamoDB for stored APOD data..."
aws dynamodb scan \
    --table-name InfiniteRawContent-dev \
    --filter-expression "begins_with(contentId, :prefix)" \
    --expression-attribute-values '{":prefix": {"S": "apod-"}}' \
    --region ${REGION} \
...
# Clean up
echo "🧹 Cleaning up..."
rm -f /tmp/apod-fetcher-response.json

echo "🎉 APOD Fetcher testing completed!"
echo ""
echo "📊 Test Summary:"
