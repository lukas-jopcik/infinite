...
# Setup ESA Fetcher Scheduled Rule for Infinite v1.0
# This script creates EventBridge rules for scheduled ESA fetching

set -e

echo "⏰ Setting up ESA Fetcher scheduled rule for Infinite v1.0..."
...
RULE_NAME="infinite-esa-weekly-${ENVIRONMENT}"
...
# 1. Create EventBridge rule for weekly ESA fetching (Mondays at 8 AM UTC)
echo "📅 Creating EventBridge rule for weekly ESA fetching..."
aws events put-rule \
    --name ${RULE_NAME} \
    --schedule-expression "cron(0 8 ? * MON *)" \
    --description "Weekly trigger for ESA Hubble data fetching on Mondays at 8 AM UTC" \
    --region ${REGION}
...
aws lambda add-permission \
    ...
    --statement-id "allow-eventbridge-esa-${ENVIRONMENT}" \
...
aws cloudwatch put-metric-alarm \
    --alarm-name "infinite-esa-fetcher-errors-${ENVIRONMENT}" \
    --alarm-description "Alert when ESA fetcher has errors" \
    --metric-name Errors \
...
aws cloudwatch put-metric-alarm \
    --alarm-name "infinite-esa-fetcher-no-invocations-${ENVIRONMENT}" \
    --alarm-description "Alert when ESA fetcher has no invocations in 7 days" \
...
echo "✅ ESA Fetcher scheduled rule setup completed successfully!"
echo ""
echo "📋 Summary:"
echo "  📅 Rule: ${RULE_NAME}"
echo "  ⏰ Schedule: Weekly on Mondays at 8 AM UTC (cron: 0 8 ? * MON *)"
echo "  ⚡ Function: ${FUNCTION_NAME}"
...
echo "  3. Verify ESA data is being fetched"
