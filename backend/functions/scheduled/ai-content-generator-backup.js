...
exports.handler = async (event) => {
    ...
    try {
        ...
        // Process each raw content item with duplicate protection
        for (const rawItem of rawContentItems) {
            try {
                console.log(`Processing raw content: ${rawItem.contentId}`);
                
                // Check if article already exists for this raw content (idempotency)
                ...
                // Try to acquire processing lock (atomic operation)
                const lockAcquired = await acquireProcessingLock(rawItem.contentId, rawItem.source);
                if (!lockAcquired) {
                    console.log(`Processing lock already acquired for ${rawItem.contentId}, skipping`);
                    continue;
                }
                ...
                try {
                    ...
                    // Update raw content status to processed (atomic operation)
                    await updateRawContentStatusAtomic(rawItem.contentId, rawItem.source, 'processed');
                    ...
                    console.error(`Error processing raw content ${rawItem.contentId}:`, processingError);
                    errorCount++;
                    
                    // Update raw content status to failed
                    try {
                        await updateRawContentStatusAtomic(rawItem.contentId, rawItem.source, 'failed');
                    } catch (updateError) {
                        console.error('Failed to update raw content status:', updateError);
                    }
                } finally {
                    // Always release the processing lock
                    await releaseProcessingLock(rawItem.contentId, rawItem.source);
                }
...
/**
 * Acquire processing lock for raw content (atomic operation)
 */
async function acquireProcessingLock(contentId, source) {
    try {
        const lockId = `processing-${contentId}-${source}`;
        ...
        const params = {
            ...
            UpdateExpression: 'SET #status = :status, processingLock = :lockId, lockExpiry = :lockExpiry, updatedAt = :updatedAt',
            ConditionExpression: '#status = :pendingStatus AND (attribute_not_exists(processingLock) OR lockExpiry < :now)',
            ExpressionAttributeNames: {
                '#status': 'status'
            },
            ExpressionAttributeValues: {
        ...
        console.log(`Processing lock acquired for ${contentId}`);
        return true;
        
    } catch (error) {
        if (error.name === 'ConditionalCheckFailedException') {
            console.log(`Processing lock already exists for ${contentId}`);
            return false;
        }
...
/**
 * Release processing lock for raw content
 */
async function releaseProcessingLock(contentId, source) {
    try {
        const params = {
            TableName: RAW_CONTENT_TABLE,
            Key: {
            ...
            UpdateExpression: 'REMOVE processingLock, lockExpiry SET updatedAt = :updatedAt',
            ExpressionAttributeValues: {
                ':updatedAt': new Date().toISOString()
            }
        };
        ...
        await dynamodb.send(new UpdateCommand(params));
        console.log(`Processing lock released for ${contentId}`);
...
/**
 * Update raw content status atomically (with lock check)
 */
async function updateRawContentStatusAtomic(contentId, source, status) {
    try {
        const params = {
            TableName: RAW_CONTENT_TABLE,
            Key: {
            ...
            UpdateExpression: 'SET #status = :status, updatedAt = :updatedAt REMOVE processingLock, lockExpiry',
            ConditionExpression: 'attribute_exists(processingLock)',
            ExpressionAttributeNames: {
                '#status': 'status'
            },
            ExpressionAttributeValues: {
        ...
        await dynamodb.send(new UpdateCommand(params));
        console.log(`Raw content status updated atomically: ${contentId} -> ${status}`);
        
    } catch (error) {
        if (error.name === 'ConditionalCheckFailedException') {
            console.log(`Processing lock not found for ${contentId}, status update skipped`);
            return;
        }
        console.error('Error updating raw content status atomically:', error);
        throw new Error('Failed to update raw content status');
    }
}

/**
 * Update raw content status (legacy function for backward compatibility)
 */
...
async function getESAHubbleContentForProcessing() {
    try {
        ...
        if (result.Items && result.Items.length > 0) {
            ...
            if (firstItem && typeof firstItem === 'object' && firstItem !== null && !firstItem.S && !firstItem.N) {
