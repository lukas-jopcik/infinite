...
- **GSI1:** `contentQuality-index` - Query by content quality score
- **GSI2:** `processingStatus-index` - Query by processing status
- **GSI3:** `validationStatus-index` - Query by validation status

**Local Secondary Indexes (LSI):**
- **LSI1:** `date-generatedAt-index` - Query by date and generation time
- **LSI2:** `date-lastUpdated-index` - Query by date and last update time
