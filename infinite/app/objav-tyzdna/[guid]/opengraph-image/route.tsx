...
import { getHubbleByGuidFromApi } from '@/lib/hubble-api'
...
export async function GET(req: NextRequest, { params }: { params: Promise<{ guid: string }> }) {
  ...
  try {
    ...
    if (!hubble) {
      throw new Error('Hubble item not found')
    }
    
    const title = hubble.title
    const date = hubble.pubDate.split('T')[0]
    const img = hubble.image_main
    ...
    return new ImageResponse(
      (
        ...
        >
          ...
          >
            <div style={{ fontSize: 28, opacity: 0.9 }}>ESA Hubble • {date}</div>
